...
public class EditController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String image_url1 = request.getParameter("image_url1");
        ...
        try {
            ...
            DAO dao = new DAO();
            dao.updateProduct(name, brand, color, price, image_url2, category_id, product_id);
            ...
            for (int i = 0; i < arrSize.length; i++) {
                if (arrSize[i] != null && arrQuantity[i] != null) {
                    dao.updateProductSize(product_id, arrSize[i], arrQuantity[i]);
                }
            }
            int[] arrImageId = dao.getImageIdByProductId(product_id);
            String[] arrImageUrl = {image_url1, image_url2, image_url3, image_url4};
            for (int i = 0; i < arrImageId.length && i < arrImageUrl.length; i++) {
                dao.updateProductImage(arrImageId[i], arrImageUrl[i]);
            }
...
//        int totalImage = dao.getTotalImage();
//        for (int i = 0; i < totalImage; i++) {
//            String[] urls = request.getParameterValues("image_url" + i);
//            if (urls != null) {
//                for (String url : urls) {
//                    imageUrls.add(url);
//                }
...
//            price = Float.parseFloat(price_String);
//
//            dao.updateProduct(name, brand, color, price, imageUrls.get(2), category_id, product_id);
//            String[] arrSize = {size_39, size_40, size_41, size_42, size_43};
//            String[] arrQuantity = {quantity_39, quantity_40, quantity_41, quantity_42, quantity_43};
//            for (int i = 0; i < arrSize.length; i++) {
//                if (arrSize[i] != null && arrQuantity[i] != null) {
...
//                dao.updateProductImage(arrImageId[i], imageUrls.get(i));
//            }
//            response.sendRedirect("manage");
//        } catch (Exception e) {
//            System.out.println(e);
